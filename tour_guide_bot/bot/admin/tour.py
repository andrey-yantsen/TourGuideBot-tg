...
from telegram.ext import ContextTypes, ConversationHandler, CommandHandler, CallbackQueryHandler, MessageHandler, filters, CallbackContext
...
class TourCommandHandler(AdminProtectedBaseHandlerCallback):
            ...
            ConversationHandler(
                ...
                states={
                    ...
                    cls.STATE_TOUR_ADD_CONTENT: [
                        ...
                        ConversationHandler(
                            entry_points=[MessageHandler(filters.AUDIO & ~filters.UpdateType.EDITED,
                                                         cls.partial(cls.translation_section_content_add_audio))],
                            states={
                                cls.STATE_TOUR_AUDIO_CONVERT_CONFIRMATION: [
                                    CallbackQueryHandler(cls.partial(
                                        cls.translation_section_content_add_audio_convert), 'convert_audio'),
                                    CallbackQueryHandler(cls.partial(
                                        cls.translation_section_content_add_store_audio), 'store_audio_as_is'),
                                    CallbackQueryHandler(cls.partial(
                                        cls.translation_section_content_add_mute_and_store_audio), 'disable_conversion_and_store_audio_as_is'),
                                ],
                                cls.STATE_TOUR_AUDIO_CONVERT_VOICE_CHECK: [
                                    ...
                                    CallbackQueryHandler(cls.partial(
                                        cls.translation_section_content_add_store_audio), 'store_audio_as_is'),
                                    CallbackQueryHandler(cls.partial(
                                        cls.translation_section_content_add_store_voice), 'store_voice'),
                                ],
                            },
                            fallbacks=[
                                CallbackQueryHandler(cls.partial(cls.cancel_audio_conversion), 'cancel'),
    ...
    def cleanup_context_audio_conversion(self, context: ContextTypes.DEFAULT_TYPE):
        for key in ('audio_message_id', 'audio_file_id', 'audio_caption', 'voice_file_path'):
            if key in context.user_data:
                del context.user_data[key]
    ...
    async def translation_section_content_add(self, message_id: int, context: ContextTypes.DEFAULT_TYPE, message_type: MessageType,
                                              text: str | None = None, media_group_id: str | None = None, file_id: str | None = None,
                                              file_caption=None, location=None):
        is_first = True
    ...
    async def translation_section_content_add_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.translation_section_content_add(update.message.message_id, context, MessageType.text, text=update.message.text_markdown_v2_urled)
        language = await self.get_language(update, context)
    ...
    async def translation_section_content_add_location(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.translation_section_content_add(update.message.message_id, context, MessageType.location, location={
    ...
    async def translation_section_content_add_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.translation_section_content_add(update.message.message_id, context, MessageType.voice,
                                                   file_id=update.message.voice.file_id,
                                                   file_caption=update.message.caption_markdown_v2_urled)

        await self._report_voice_saved(update, context)

    async def _report_voice_saved(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        language = await self.get_language(update, context)
    ...
    async def translation_section_content_add_video_note(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.translation_section_content_add(update.message.message_id, context, MessageType.video_note,
    ...
    async def convert_audio(self, context: CallbackContext):
        ...
        try:
            ...
            log.exception('Failed to download file %s', context.user_data['audio_file_id'])
            await bot.edit_message_text(
                t(language).pgettext('admin-tours',
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton(
                        t(language).pgettext('admin-tours', 'Store the original audio'),
        ...
        try:
            process = ffmpeg.input(original_audio_path).output(
                destination_path, acodec='libopus', **{'b:a': '192000'}).run_async()
            ...
            log.exception('Failed to convert file %s', context.user_data['audio_file_id'])
            await bot.edit_message_text(
                t(language).pgettext('admin-tours',
        ...
        try:
            message = await bot.send_voice(context.job.data['chat_id'], open(destination_path, 'rb'),
                                           reply_markup=InlineKeyboardMarkup([
                                               [InlineKeyboardButton(
                                                   t(language).pgettext('bot-generic', 'Store the voice message'),
            ...
            log.exception('Failed to upload voice message %s', context.user_data['audio_file_id'])
            await bot.edit_message_text(
                t(language).pgettext('admin-tours',
    ...
    async def translation_section_content_add_audio_convert(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        language = await self.get_language(update, context)
        context.job_queue.run_once(self.convert_audio, 0, data={
            'message_id': update.callback_query.message.id,
            'language': language,
            'chat_id': update.effective_chat.id,
            'user_id': update.effective_user.id,
        }, chat_id=update.effective_chat.id, user_id=update.effective_user.id)
        await self.edit_or_reply_text(update, context, t(language).pgettext('admin-tours',
                                                                            'Starting the audio conversion process...'))
        return self.STATE_TOUR_AUDIO_CONVERT_VOICE_CHECK

    async def translation_section_content_add_store_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.translation_section_content_add(context.user_data['audio_message_id'], context, MessageType.voice,
    ...
    async def translation_section_content_add_store_audio(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.translation_section_content_add(context.user_data['audio_message_id'], context, MessageType.audio,
    ...
    async def translation_section_content_add_mute_and_store_audio(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        language = await self.get_language(update, context)
        await self.reply_text(update, context, t(language).pgettext('admin-tours',
    ...
    async def _report_audio_saved(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        language = await self.get_language(update, context)
    ...
    async def translation_section_content_add_audio(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not update.message.media_group_id:
            ...
            if audio_to_voice_state.is_enabled:
                ...
                context.user_data['audio_caption'] = update.message.caption_markdown_v2_urled
                ...
                await update.message.reply_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        ...
                        [InlineKeyboardButton(
                            t(language).pgettext('admin-tours', 'No, and do not ask again.'),
        ...
        is_first = await self.translation_section_content_add(update.message.message_id, context, MessageType.audio,
    ...
    async def translation_section_content_add_video(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        is_first = await self.translation_section_content_add(update.message.message_id, context, MessageType.video,
                                                              media_group_id=update.message.media_group_id,
    ...
    async def translation_section_content_add_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        is_first = await self.translation_section_content_add(update.message.message_id, context, MessageType.photo,
                                                              media_group_id=update.message.media_group_id,
    ...
    async def request_delete_tour_confirmation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.callback_query.edit_message_text(t(user.language).pgettext(
            'admin-tours', 'Do you really want to delete the tour "{0}"?'.format(get_tour_title(tour, user.language, context))),
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [
                    InlineKeyboardButton(t(user.language).pgettext('bot-generic',
    ...
    async def request_tour_title(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.edit_or_reply_text(update, context, t(user.language).pgettext('admin-tour', 'Please send me the'
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(t(user.language).pgettext(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [
                    InlineKeyboardButton(t(user.language).pgettext('admin-tours',

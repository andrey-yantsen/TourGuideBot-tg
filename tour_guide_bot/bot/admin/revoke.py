...
class RevokeCommandHandler(AdminProtectedBaseHandlerCallback):
    ...
    def get_handlers(cls):
        return [
            ConversationHandler(
                ...
                states={
                    cls.STATE_TOUR: [
                        CallbackQueryHandler(cls.partial(cls.tour), '^revoke_tour:(\d+)$'),
                    ],
                    ...
                    cls.STATE_REVOKE: [
                        CallbackQueryHandler(cls.partial(cls.revoke), '^revoke_confirm:(\d+)$'),
                    ],
                },
                fallbacks=[
    ...
    async def revoke(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        purchase = await self.db_session.scalar(select(BoughtTours).where((BoughtTours.guest == guest)
    ...
    async def phone_number(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not guest:
            await update.message.reply_text(t(user.language).pgettext('admin-revoke',
                                        ...
                                        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                            [
                                                InlineKeyboardButton(t(user.language).pgettext('bot-generic',
                                                                                               'Yes'), callback_data='revoke_confirm:%d' % (tour.id, )),
    ...
    async def tour(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user = await self.get_user(update, context)
        await update.callback_query.edit_message_text(t(user.language).pgettext('admin-revoke',
